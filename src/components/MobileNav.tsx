                ...
                <div className="w-full flex flex-col gap-4">
                    ...
                    {products.length > 0 && (
                        <div className="grid grid-cols-3 gap-2 w-full mt-2">
                            {products.map((p) => (
                                <div key={p.id} className="bg-white rounded-xl overflow-hidden flex flex-col p-2 h-full shadow-sm border border-gray-100">
                                    ...
                                    <a href={`/san-pham/${p.link}`} className="mt-auto block text-center bg-red-500 text-white py-1.5 rounded-lg text-[9px] font-bold uppercase">Xem ngay</a>
                                </div>
                            ))}
                        </div>
                    )}
        ...
        <>
            ...
            {(isChatOpen || isMenuOpen) && (
                <div
                    ...
                    onClick={() => { setIsChatOpen(false); setIsMenuOpen(false); }}
                />
            )}
            ...
            <div className="fixed bottom-0 right-0 md:bottom-5 md:right-4 flex items-end justify-end gap-2 z-[9999] w-full md:w-auto pointer-events-none">
                {/* Chat Window */}
                {isChatOpen && (
                    <div className="flex flex-col overflow-hidden bg-[#FFF0F1] shadow-2xl fixed bottom-0 left-0 w-full h-[90vh] rounded-t-[2.5rem] z-[10000] md:relative md:w-[420px] md:h-[95vh] md:rounded-3xl border border-red-100 animate-in slide-in-from-bottom-5 pointer-events-auto">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6 no-scrollbar bg-[#FFF0F1]">
                            {messages.map((m, i) => (
                                <div key={i} className={`flex gap-2 items-start ${m.senderType === "USER" ? "flex-row-reverse" : "flex-row"}`}>
                                    ...
                                    <div className={`flex flex-col ${m.senderType === "USER" ? "items-end" : "items-start"} max-w-full`}>
                            ...
                            {isTyping && <div className="flex gap-2 items-center text-gray-400 text-xs italic ml-2"><Bot size={14} className="animate-bounce" /> <span className="bg-white text-black rounded-full p-2 animate-pulse px-4"> Đang trả lời...</span></div>}
                            <div ref={messagesEndRef} />
                        </div>

                        <div className="p-4 bg-white border-t border-red-50 shrink-0">
                            <div className="flex gap-2 items-center bg-gray-50 rounded-2xl px-4 py-1.5 border border-gray-100">
                                <input
                                ...
                                />
                                <button onClick={sendMessage} className="bg-red-500 p-2.5 rounded-xl text-white active:scale-90"><Send size={18} /></button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className={`flex flex-col gap-3 pb-5 pr-4 md:p-0 pointer-events-auto transition-all ${isChatOpen ? "hidden md:flex" : "flex"}`}>
                    <div className={`flex flex-col gap-3 transition-all duration-300 ${isMenuOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 pointer-events-none md:opacity-100 md:translate-y-0 md:pointer-events-auto"}`}>
                        ...
                        >
                            <MessageCircle size={20} /><span>Tư Vấn</span>
                        ...
                        >
                            <Headset size={20} /><span>Liên hệ</span>
                        </button>
                    </div>
